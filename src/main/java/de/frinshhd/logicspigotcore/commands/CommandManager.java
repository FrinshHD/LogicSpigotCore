package de.frinshhd.logicspigotcore.commands;
            ...
            if (className.contains(canonicalName)) {
                try {
                    ...
                    if (basicCommandClass.isAssignableFrom(cls)) {
                        ...
                        LogicSpigotCoreMain.getInstance().getLogger().info("[DynamicCommands] Trying to register command " + className);
                        ...
                        try {
                            ...
                            commandMap.register(LogicSpigotCoreMain.getInstance().getName(), basicCommand);
                        } catch (Exception e) {
                            LogicSpigotCoreMain.getInstance().getLogger().warning("[DynamicCommands] Error registering command " + className + " " + e);
                            return;
                        }

                        LogicSpigotCoreMain.getInstance().getLogger().info("[DynamicCommands] Successfully registered command " + className);
                    }
                } catch (ClassNotFoundException | IllegalAccessException | InstantiationException |
                         InvocationTargetException | IllegalArgumentException e) {
                    LogicSpigotCoreMain.getInstance().getLogger().warning("[DynamicCommands] Error loading command " + className + " " + e);
                }
            }
        }
    }
            ...
            if (className.contains(canonicalName)) {
                try {
                    ...
                    if (basicSubCommandClass.isAssignableFrom(cls)) {
                        ...
                        LogicSpigotCoreMain.getInstance().getLogger().info("[DynamicCommands] Trying to register subcommand " + className);
                        ...
                        LogicSpigotCoreMain.getInstance().getLogger().info("[DynamicCommands] Successfully registered subcommand " + className);
                    }
                } catch (ClassNotFoundException | IllegalAccessException | InstantiationException |
                         InvocationTargetException | IllegalArgumentException e) {
                    LogicSpigotCoreMain.getInstance().getLogger().warning("[DynamicCommands] Error loading subcommand " + className + " " + e);
                }
            }
        }
    }
